...
function Snorql() {
	var m = document.location.href.match(/^([^?]*(?::(\d+)*)\/)[^?]*snorql\//);
    ...
    this.start = function() {
        ...
        var dummy = this;
   	    ...
   	    if (match) {
	        ...
	        } else if (match[1].toUpperCase() == 'CONSTRUCT' || match[1].toUpperCase() == 'DESCRIBE' || match[1].toUpperCase() == 'EXPLAIN') { // construct describe explain
