 ...
 *
 */
public class E_Negative extends XLExpr1 {

	/**
	 * constructor
	 */
	public E_Negative(XLExpr arg) {
		super(arg);
	}
	...
	public XLExprValue<?> eval(ExecutionContext context) throws XLWrapException, XLWrapEOFException {
		...
		if (number instanceof E_Long)
			return new E_Long(- ((E_Long) number).getValue());
		else if (number instanceof E_Double)
			return new E_Double(- ((E_Double) number).getValue());
		else
			throw new XLWrapException("Cannot apply negative value to " + arg);
	}

	@Override
	public XLExpr copy() {
		return new E_Negative(arg.copy());
	}
