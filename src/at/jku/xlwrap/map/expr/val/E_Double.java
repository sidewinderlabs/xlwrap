 ...
 *
 */
public class E_Double extends XLExprNumber<Double> {
	 ...
	 * @param value
	 */
	public E_Double(Double value) {
		super(value);
	}

	@Override
	public XLExpr copy() {
		return new E_Double(new Double(value));
	}
	
	@Override
	public XLExprNumber<?> add(XLExprNumber<?> other) throws XLWrapException {
		if (other instanceof E_Long)
			return new E_Double(this.value + (Long) other.value);
	...
	public XLExprNumber<?> divide(XLExprNumber<?> other) throws XLWrapException {
		if (other instanceof E_Long) {
			return new E_Double(value / ((E_Long) other).getValue());
		} else if (other instanceof E_Double)
