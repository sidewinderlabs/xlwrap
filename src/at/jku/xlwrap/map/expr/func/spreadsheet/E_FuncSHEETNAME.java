 ...
 *
 */
public class E_FuncSHEETNAME extends XLExprFunction {

	/**
	 * default constructor
	 */
	public E_FuncSHEETNAME() {
	}
	
	/**
	 * constructor
	 */
	public E_FuncSHEETNAME(XLExpr arg) {
		addArg(arg);
	}
	...
	public XLExprValue<?> eval(ExecutionContext context) throws XLWrapException {
		...
		if (args.size() == 0)
			return new E_String(context.getActiveTemplate().getSheetName());
		
		else {	
			if (!(args.get(0) instanceof E_RangeRef))
				throw new XLWrapException("Argument " + args.get(0) + " of " + FunctionRegistry.getFunctionName(E_FuncSHEETNAME.class) + " must be a cell range reference.");
			Range absolute = ((E_RangeRef) args.get(0)).getRange().getAbsoluteRange(context);
			if (!(absolute instanceof CellRange))
				throw new XLWrapException("Argument " + args.get(0) + " of " + FunctionRegistry.getFunctionName(E_FuncSHEETNAME.class) + " must be a cell range reference.");
			
			return new E_String(((CellRange) absolute).getSheetName());
		}
	}
