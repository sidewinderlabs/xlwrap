 ...
 *
 */
public class E_FuncROW extends XLExprFunction {
	 ...
	 * 
	 * @param range
	 * @param syntax a int value (if null or 0 return 1-based row (Excel syntax), otherwise return zero-based integer)
	 */
	public E_FuncROW(XLExpr range, XLExpr syntax) {
		addArg(range);
		addArg(syntax);
	}
	...
	public XLExprValue<?> eval(ExecutionContext context) throws XLWrapException, XLWrapEOFException {
		...
		if (!(args.get(0) instanceof E_RangeRef))
			throw new XLWrapException("Argument " + args.get(0) + " of " + FunctionRegistry.getFunctionName(E_FuncROW.class) + " must be a cell range reference.");
		...
		if (args.size() == 1 || args.size() > 1 && TypeCast.toInteger(args.get(1).eval(context), false) == 1)
			return new E_Long((long) ((CellRange) absolute).getRow() + 1);
		else // no syntax specified or syntax != 1
			return new E_Long((long) ((CellRange) absolute).getRow());
	}
